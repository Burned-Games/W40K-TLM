...
function on_ready() 
    ...
    if prefabShield then
        ...
        if actualshield then
            ...
            -- Asegurarse que el renderizador está activo y visible
            if rendererComp then
                rendererComp.enabled = true
                rendererComp.visible = true
                -- Copiar la mesh y material del prefab
                ...
                if prefabRenderer then
                    ...
                    rendererComp.material = prefabRenderer.material
                end
...
function shield_state(dt)
    ...
    if actualshield~= nil and transformShield ~= nil and canUseShield then
        ...
        elseif rangeDistance > maxDistance then
            ...
            if not enemiesScript.shield_state and canUseShield then
                if transformShield ~= nil then
                    transformActualShield.position = enemyRangeTransf.position
                    print("ActualShield en el enemy" .. transformActualShield.position.x .. " " .. transformActualShield.position.y .. " " .. transformActualShield.position.z)
                    if enemyRangeEntity ~= nil then
                        if enemiesScript ~= nil then
                            ...
                            print("Shield Health: " .. enemiesScript.shieldHealth)
                            shieldTimer = 0
                            find_range_enemies()
                            check_enemies_shield_status()
                        end
