...
function on_ready()
    ...
    maxAmmoTextComponent = current_scene:get_entity_by_name("Balas"):get_component("UITextComponent")
...
function updateAmmoText()
    if currentWeapon == 1 then
        ...
        maxAmmoTextComponent:set_text(tostring(maxAmmoWeapon1))
    else
        ammoTextComponent:set_text(tostring(currentAmmoWeapon2))
        maxAmmoTextComponent:set_text(tostring(maxAmmoWeapon2))
    end
end
...
function update_health_display()
    ...
    lifeFullComponent:set_size(Vecto2.new(-94, -playerScript.playerHealth))
